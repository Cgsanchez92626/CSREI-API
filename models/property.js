...
const propertySchema = new mongoose.Schema({
  ...
  parcelNumber: {
    ...
    validate: {
      ...
      message: (props) =>
        `${props.value} is not a valid parcel number format! Must be in the format "9999-999-999"`,
    },
  },
  yearBuilt: {
    type: Number,
    validate: {
      validator: function (v) {
        ...
        // Check if v is a valid 4-digit number
        return /^\d{4}$/.test(v.toString());
      },
      message: (props) =>
        `${props.value} is not a valid Year format! Must be a 4-digit number`,
    },
  },
  propertyType: {
    type: String,
